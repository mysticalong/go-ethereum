...
type ChainConfig struct {
	...
	Polo   *PoloConfig   `json:"polo,omitempty"`
}
...
// PoloConfig is the consensus engine configs for proof-of-authority based sealing.
type PoloConfig struct {
	Period uint64   `json:"period"` // Number of seconds between blocks to enforce
	Epoch  uint64   `json:"epoch"`  // Epoch length to sync signers and checkpoint
	PCRs   []string `json:"PCRs"`
}

// String implements the stringer interface, returning the consensus engine details.
func (c *PoloConfig) String() string {
	s := fmt.Sprintf("Period=%d", c.Period)
	s += fmt.Sprintf("Epoch=%d", c.Epoch)
	for i, v := range c.PCRs {
		s += fmt.Sprintf("PCR%d=%s", i, v)
	}
	return "Polo:\n" + s
}
